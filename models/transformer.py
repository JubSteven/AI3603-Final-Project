...
"""
    ViT for patch processing in CUT++
    Adapted by pengxiang from https://github.com/gupta-abhay/pytorch-vit/blob/main/vit/transformer.py
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        assert isinstance(mlp_ratio, float), "MLP ratio should be an integer for valid "
        ...
        for _ in range(depth):
            self.layers.append(
                nn.ModuleList([
                    ...
                    ) if not revised else FeedForward(
                        dim,
                        mlp_dim,
                        dropout_rate=dropout,
                        revised=True,
