...
class Upsample(nn.Module):
    ...
    def forward(self, inp):
        ret_val = F.conv_transpose2d(self.pad(inp),
                                     self.filt,
                                     stride=self.stride,
...
def define_F(input_nc,
    ...
    elif netF == "attn_mlp_sample":
        net = AttnPatchSampleF(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc)
    else:
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            if num_patches > 0:
                ...
                x_sample = feat_reshape[:, patch_id, :].flatten(0, 1)  # [B*num_patches, C]
            else:
...
class AttnPatchSampleF(nn.Module):
    ...
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(AttnPatchSampleF, self).__init__()
        self.l2norm = Normalize(2)
        self.use_mlp = use_mlp
        self.nc = nc  # hard-coded
        self.mlp_init = False
        ...
        self.transformer = Transformer(dim=nc, depth=1, heads=2)
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            # >>>>>>>>> @pengxiang added transformer part for feature processing
            x_sample = x_sample.view(B, -1, self.nc)
            x_sample = self.transformer(x_sample)
            x_sample = x_sample.view(-1, self.nc)
...
class G_Resnet(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, nz, num_downs, n_res, ngf=64, norm=None, nl_layer=None):
        ...
        if nz == 0:
            ...
            self.dec = Decoder_all(n_downsample,
                                   n_res,
                                   self.enc_content.output_dim,
                                   output_nc,
                                   norm=norm,
