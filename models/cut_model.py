...
class CUTModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--netF',
                            type=str,
                            default='attn_mlp_sample',
                            choices=['sample', 'reshape', 'mlp_sample', 'attn_mlp_sample'],
    ...
    def __init__(self, opt):
        ...
        # netG will return a list of feature maps from layers specified in opt.nce_layers
        # for example, if the NCEloss want to be calculated from layers 0, 4, 8, 12, 16,
        # then netG will return a list of 5 feature maps.
        self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.normG, not opt.no_dropout,
                                      ...
                                      opt)
        # netF is responsible for sampling the feature map returned by netG to form a set of patches
        self.netF = networks.define_F(opt.input_nc, opt.netF, opt.normG, not opt.no_dropout, opt.init_type,
                                      opt.init_gain, opt.no_antialias, self.gpu_ids, opt)
        """
        Feature size of ResNet-9Blocks of layers 0, 4, 8, 12, 16
            torch.Size([B, 262, 262])
    ...
    def optimize_parameters(self):
        ...
        if self.opt.netF == 'mlp_sample' or self.opt.netF == 'attn_mlp_sample':
    ...
    def forward(self):
        ...
        self.real = torch.cat(
            (self.real_A, self.real_B), dim=0) if self.opt.nce_idt and self.opt.isTrain else self.real_A
    ...
    def calculate_NCE_loss(self, src, tgt):
        ...
        # Original features, a list of length self.nce_layers
        ...
        # Sample features from feat_k and feat_q to form a set of patches
